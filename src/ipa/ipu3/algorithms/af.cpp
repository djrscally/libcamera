...
void Af::process(IPAContext &context, const ipu3_uapi_stats_3a *stats)
{
	...
	for (z = 0; z < (IPU3_UAPI_AF_Y_TABLE_MAX_SIZE) / 4; z++) {
		printf("%d, ", ((y_table_item_t *)stats->af_raw_buffer.y_table)[z].y2_avg);
		total = total + ((y_table_item_t *)stats->af_raw_buffer.y_table)[z].y2_avg;
		if (((y_table_item_t *)stats->af_raw_buffer.y_table)[z].y2_avg == 0)
			break;
	}
	mean = total / z;

	/* Calculate the variance of every AF statistic value. */
	for (z = 0; z < (IPU3_UAPI_AF_Y_TABLE_MAX_SIZE) / 4 && ((y_table_item_t *)stats->af_raw_buffer.y_table)[z].y2_avg != 0; z++) {
		var_sum = var_sum + ((((y_table_item_t *)stats->af_raw_buffer.y_table)[z].y2_avg - mean) * (((y_table_item_t *)stats->af_raw_buffer.y_table)[z].y2_avg - mean));
		if (((y_table_item_t *)stats->af_raw_buffer.y_table)[z].y2_avg == 0)
			break;
	}
